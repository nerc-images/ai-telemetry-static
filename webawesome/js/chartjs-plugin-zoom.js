 ...
 */
// MODIFIED // (function (global, factory) {
// MODIFIED // typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('chart.js'), require('hammerjs'), require('chart.js/helpers')) :
// MODIFIED // typeof define === 'function' && define.amd ? define(['chart.js', 'hammerjs', 'chart.js/helpers'], factory) :
// MODIFIED // (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.ChartZoom = factory(global.Chart, global.Hammer, global.Chart.helpers));
// MODIFIED // })(this, (function (chart_js, Hammer, helpers) { 'use strict';
...
function getEnabledScalesByPoint(options, point, chart) {
  ...
  const enabledScales = [];
  /* MODIFIED */Chart.helpers.each(chart.scales, function(scaleItem) {
...
function getLimit(state, scale, scaleLimits, prop, fallback) {
  ...
  if (limit === 'original') {
    ...
    limit = /* MODIFIED */Chart.helpers.valueOrDefault(original.options, original.scale);
  }
  return /* MODIFIED */Chart.helpers.valueOrDefault(limit, fallback);
}
...
function fixRange(range, {min, max, minLimit, maxLimit}, originalLimits) {
  ...
  if (/* MODIFIED */Chart.helpers.almostEquals(min, origMin, epsilon)) {
    min = origMin;
  }
  if (/* MODIFIED */Chart.helpers.almostEquals(max, origMax, epsilon)) {
    max = origMax;
  }
  if (min < minLimit) {
    min = minLimit;
...
function removeMissingScales(limits, scales) {
  /* MODIFIED */Chart.helpers.each(limits, (opt, key) => {
    if (!scales[key]) {
      delete limits[key];
    }
  });
...
function storeOriginalScaleLimits(chart, state) {
  ...
  /* MODIFIED */Chart.helpers.each(scales, function(scale) {
...
function doZoom(scale, amount, center, limits) {
  ...
  /* MODIFIED */Chart.helpers.callback(fn, [scale, amount, center, limits]);
}
function doZoomRect(scale, from, to, limits) {
  const fn = zoomRectFunctions[scale.type] || zoomRectFunctions.default;
  /* MODIFIED */Chart.helpers.callback(fn, [scale, from, to, limits]);
}
...
function zoom(chart, amount, transition = 'none', trigger = 'api') {
  ...
  /* MODIFIED */Chart.helpers.each(enabledScales || chart.scales, function(scale) {
  ...
  /* MODIFIED */Chart.helpers.callback(zoomOptions.onZoom, [{chart, trigger}]);
}
function zoomRect(chart, p0, p1, transition = 'none', trigger = 'api') {
  ...
  /* MODIFIED */Chart.helpers.each(chart.scales, function(scale) {
    if (scale.isHorizontal() && xEnabled) {
...
function zoomScale(chart, scaleId, range, transition = 'none', trigger = 'api') {
  ...
  chart.update(transition);
  /* MODIFIED */Chart.helpers.callback(state.options.zoom?.onZoom, [{chart, trigger}]);
}
function resetZoom(chart, transition = 'default') {
  ...
  chart.update(transition);
  /* MODIFIED */Chart.helpers.callback(state.options.zoom.onZoomComplete, [{chart}]);
}
function getOriginalRange(state, scaleId) {
  ...
  return /* MODIFIED */Chart.helpers.valueOrDefault(max.options, max.scale) - /* MODIFIED */Chart.helpers.valueOrDefault(min.options, min.scale);
}
...
function panScale(scale, delta, limits, state) {
  ...
  if (/* MODIFIED */Chart.helpers.sign(storedDelta) === /* MODIFIED */Chart.helpers.sign(delta)) {
    delta += storedDelta;
  }
  const fn = panFunctions[scale.type] || panFunctions.default;
  if (/* MODIFIED */Chart.helpers.callback(fn, [scale, delta, limits])) {
    panDelta[scale.id] = 0;
  } else {
...
function pan(chart, delta, enabledScales, transition = 'none') {
  ...
  chart.update(transition);
  /* MODIFIED */Chart.helpers.callback(onPan, [{chart}]);
}
...
function getPointPosition(event, chart) {
  ...
  return /* MODIFIED */Chart.helpers.getRelativePosition(event, chart);
}
function zoomStart(chart, event, zoomOptions) {
  const {onZoomStart, onZoomRejected} = zoomOptions;
  if (onZoomStart) {
    const point = getPointPosition(event, chart);
    if (/* MODIFIED */Chart.helpers.callback(onZoomStart, [{chart, event, point}]) === false) {
      /* MODIFIED */Chart.helpers.callback(onZoomRejected, [{chart, event}]);
      return false;
    }
  }
}
function mouseDown(chart, event) {
  if (chart.legend) {
    const point = /* MODIFIED */Chart.helpers.getRelativePosition(event, chart);
    if (/* MODIFIED */Chart.helpers._isPointInArea(point, chart.legend)) {
      return;
    }
  }
  const state = getState(chart);
  ...
  if (
    ...
    return /* MODIFIED */Chart.helpers.callback(zoomOptions.onZoomRejected, [{chart, event}]);
  }
...
function mouseUp(chart, event) {
  ...
  /* MODIFIED */Chart.helpers.callback(onZoomComplete, [{chart}]);
}
function wheelPreconditions(chart, event, zoomOptions) {
  if (keyNotPressed(getModifierKey(zoomOptions.wheel), event)) {
    /* MODIFIED */Chart.helpers.callback(zoomOptions.onZoomRejected, [{chart, event}]);
    return;
  }
  if (zoomStart(chart, event, zoomOptions) === false) {
    return;
...
function addDebouncedHandler(chart, name, handler, delay) {
  if (handler) {
    getState(chart).handlers[name] = debounce(() => /* MODIFIED */Chart.helpers.callback(handler, [{chart}]), delay);
  }
}
// MODIFIED //
function onZoomComplete2({chart}) {
};
// MODIFIED //
function addListeners(chart, options) {
  ...
  if (wheelOptions.enabled) {
    ...
    addDebouncedHandler(chart, 'onZoomComplete', /* MODIFIED */onZoomComplete2/* MODIFIED */, 250);
  } else {
...
function createEnabler(chart, state) {
  return function(recognizer, event) {
    ...
    if (!state.panning && event.pointerType === 'mouse' && (
      ...
      /* MODIFIED */Chart.helpers.callback(panOptions.onPanRejected, [{chart, event}]);
      return false;
    }
    return true;
  };
...
function startPinch(chart, state, event) {
  if (state.options.zoom.pinch.enabled) {
    const point = /* MODIFIED */Chart.helpers.getRelativePosition(event, chart);
    if (/* MODIFIED */Chart.helpers.callback(state.options.zoom.onZoomStart, [{chart, event, point}]) === false) {
      state.scale = null;
      /* MODIFIED */Chart.helpers.callback(state.options.zoom.onZoomRejected, [{chart, event}]);
    } else {
      state.scale = 1;
    }
  }
...
function startPan(chart, state, event) {
  ...
  if (/* MODIFIED */Chart.helpers.callback(onPanStart, [{chart, event, point}]) === false) {
    return /* MODIFIED */Chart.helpers.callback(onPanRejected, [{chart, event}]);
  }
...
function endPan(chart, state) {
  ...
  if (state.panning) {
    state.panning = false;
    state.filterNextClick = true;
    /* MODIFIED */Chart.helpers.callback(state.options.pan.onPanComplete, [{chart}]);
  }
}
...
// MODIFIED // chart_js.Chart.register(Zoom);
// MODIFIED // 
// MODIFIED // return Zoom;
// MODIFIED // 
// MODIFIED // }));
